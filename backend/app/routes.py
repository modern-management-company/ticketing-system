...
def get_team_members():
    """Get all team members under the current user's management"""
    user_id = get_jwt_identity()
    ...
    # For general_manager or manager, return users assigned to their properties
    elif current_user.role in ['general_manager', 'manager']:
        # Get properties managed by this user
        managed_properties = []
        for user_property in current_user.user_properties:
            if user_property.is_manager:
                managed_properties.append(user_property.property_id)
        
        # Also include properties where user is explicitly set as manager
        property_managers = PropertyManager.query.filter_by(user_id=user_id).all()
        for pm in property_managers:
            if pm.property_id not in managed_properties:
                managed_properties.append(pm.property_id)
        
        # Find all users assigned to these properties
        team_members = User.query.join(UserProperty).filter(
            UserProperty.property_id.in_(managed_properties)
        ).all()
    else:
        # Regular users can only see themselves
    ...
    return jsonify([{
        ...
        } for up in user.user_properties]
    } for user in team_members]), 200

@app.route('/properties/<int:property_id>/team', methods=['GET'])
@jwt_required()
@handle_errors
def get_property_team(property_id):
    """Get team members for a specific property"""
    user_id = get_jwt_identity()
    current_user = User.query.get(user_id)
    ...
    # Check if user has permission to view this property's team
    if current_user.role != 'super_admin':
        user_property = UserProperty.query.filter_by(
            user_id=user_id, 
            property_id=property_id
        ...
        # Also check PropertyManager table
        property_manager = PropertyManager.query.filter_by(
            user_id=user_id,
            property_id=property_id
        ).first()
        
        if not (user_property and user_property.is_manager) and not property_manager:
            return jsonify({'message': 'Not authorized to view this property\'s team'}), 403
...
@app.route('/properties/managed', methods=['GET'])
@jwt_required()
@handle_errors
def get_managed_properties_for_user():
    ...
    # For super_admin, return all properties
    if current_user.role == 'super_admin':
        properties = Property.query.all()
    # For general_manager or manager, return properties they manage
    elif current_user.role in ['general_manager', 'manager']:
        # Get properties managed by this user
        properties = []
        for user_property in current_user.user_properties:
            if user_property.is_manager:
                properties.append(user_property.property)
        
        # Also get properties where they are explicitly set as manager
        property_managers = PropertyManager.query.filter_by(user_id=user_id).all()
        for pm in property_managers:
            if pm.property not in properties:
                properties.append(pm.property)
    else:
        # Regular users don't manage properties
        properties = []
